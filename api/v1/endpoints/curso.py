...
async def get_curso(curso_id: int, db: AsyncSession = Depends(get_session)):
    async with db as session:
        ...
        result = await session.execute(query)
        curso: CursoModel = result.scalars().unique().one_or_none()
...
async def put_curso(curso_id: int, curso: CursoSchemaCreate, db: AsyncSession = Depends(get_session)):
    async with db as session:
        query = select(CursoModel).filter(CursoModel.id == curso_id)
        result = await session.execute(query)
        curso_up: CursoModel = result.scalars().unique().one_or_none()
...
async def delete_curso(curso_id: int, db: AsyncSession = Depends(get_session)):
    async with db as session:
        query = select(CursoModel).filter(CursoModel.id == curso_id)
        result = await session.execute(query)
        curso_del: CursoModel = result.scalars().unique().one_or_none()
